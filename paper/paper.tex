...
\end{wrapfigure}
\MyComment{For instance, the purpose of rule option \CodeIn{msg} is to print on
...
\caption{ICMP Packet Structure}
\label{fig:overview}
\end{figure*}

\Luc{Os campos Flags e Time to live no header IP são mapeados para as opções fragbits e ttl, respectivamente. Os campos Type e Code no header ICMP são mapeados para as opções itype e icode. \\
...
this attack. This rule was obtained from the \Luc{Emerging Threats Open Ruleset}.
...
number of these options per rule (up to 10) for the \Luc{Emerging Threats Open Ruleset}. Rules without \CodeIn{content} options is an
...
\tname{} leverages public datasets\Fix{cite cite} of negative traffic
to guide a search for more general rules, hence addressing the
...
\textbf{OUTPUT:} Set of minimal rules $R$\\
\vspace{-3ex}
%\noindent\begin{minipage}{\textwidth}
\captionof{algorithm}{Minimization algorithm}\label{algo1}
\begin{algorithmic}[1]
%  \Procedure{MinRule}{$or$}\Comment{The g.c.d. of a and b}
