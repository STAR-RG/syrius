...
negative traffic and examples of trusted rules to create
...
early evidence showing that \tname\ is very promising. For example, it
was able to rank the correct rule among the first five rules of the
reported ranking in \percTopFiveRanking\ of the cases we analyzed.
...
where \tname\ performs worst---jboss \Fix{Remover jboss}. We measured the position of the
...
\Luc{eh possivel incluir contents em regras icmp tb. o q a secao 2.3 
...
\Fix{...what...} to \Fix{...calibrate parameters of?....}
...
complementary.
\Gui{Anomaly-based intrusion detection. Cordy and colleagues[28] 
propose the use of a search-based genetic algorithm to not only 
generate attack schemes capable of penetrating the tested anomaly-based 
NIDS, but also repairing the NIDS to identify the given attack scheme. 
Although our approach and theirs use machine intelligence to detect 
attacks, as discussed on Section 2 anomaly-based and rule-based NIDS 
are complementary.}
...
%%  LocalWords:  Suricata SolarWinds QRadar Rulesets Ruleset rulesets
%%  LocalWords:  pipelined pcap OISF IPS icmp msg classtype sid arg
%%  LocalWords:  clp http ip bitvector flagbits IPs NMAP dsize itype
%%  LocalWords:  icode ruleset th HtmlAdaptor pseudocode prce dataset
%%  LocalWords:  flowbits len tcp uri RQ lrrrr Cand UDP ColdFusion
%%  LocalWords:  iisadmin JBoss JMX WeBid LFI htaccess idq DoS AREC
%%  LocalWords:  Basebuilder PESC RCE NetReseC Nikto WireShark hping
%%  LocalWords:  Bigflows TcpReplay lrrrrrr boxplot Metasploit ACK
%%  LocalWords:  Vollmer Hallahan Cordy NIDS
