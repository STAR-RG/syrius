...
whereas the contribution of a rule with 9 options would be 14.25
(=763/5,353). Note that the measurement is normalized in the 0-1
range.

\subsubsection{Number of content options.} We empirically observed that content options are very
common. As consequence, we defined a heuristic function that takes the
...
\[\frac{\mathit{rules\_per\_number\_of\_contents[len(rule)]}}{\mathit{max(rules\_per\_number\_of\_contents)}}\]
