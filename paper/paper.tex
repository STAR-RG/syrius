...
\keywords{Intrusion Detection, Suricata, Rule-based, Synthesis}
...
8 in the histogram for space. \MyComment{Rules without
...
most non-http attacks use field options only. % We selected
%\numNonContentAttacks\ non-http attacks and \numContentAttacks\ http
%attacks. The higher number of http attacks reflects their popularity.
...
function. The number corresponds to the order of hte heuristic function
...
%We discuss in the following techniques similar in goal to
%ours. Unfortunately

%% As detailed before,
%% we could not access the tools implementing the techniques for
%% comparison. Nevertheless, we found that t
