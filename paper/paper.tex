...
(=$\frac{10,111}{10,111*1}$). The value of $M_i$ for this case is 1 as
the field of the HTTP message \CodeIn{Request URI}---holding the
string ``/HtmlAdaptor''---can be accessed through the modifier
\CodeIn{http\_uri}. Conceptually, the relatively high heuristic value
...
(\CodeIn{http\_uri}). For the option \CodeIn{content: ``keep-alive''},
either \CodeIn{http\_header} or \CodeIn{http\_coxnnection} could be
