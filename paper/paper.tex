...
\emph{plausible rules}, \ie{}, rules that captures only the malicious
traffic produced by the attacks. Figure~\ref{fig:overview} shows the workflow of \tname{} as a
...
rules for $N$ times, discarding one rule option each
...
\nids\ rule that we obtained from credible sources \rui{which are?}\Luc{\cite{emerging-threats-open}}. \tname\ generates
...
\begin{table}[t!]
  ...
  \begin{tabular}{lrrrr}
    ...
    ColdFusion & 7 & 19 & 40 & 4\\
    iisadmin access & 3 & 17 & 605 & 1 \\        
    JBoss JMX & 5 & 23 & 925 & 87 \\
    scripttag & 23 & 22 & 2,221 & 1 \\
    WeBid LFI & 1 & 23 & 35,444 & 17\\    
    %%Wordpress Login & 8 & 59 & - & - \\
    .htaccess access & 5 & 17 & 852 & 1\\
    .idq access & 14 & 19 & 806 & 2 \\
    /system32/ in URI & 4 & 20 & 1881 & 2 \\
    Basebuilder LFI & 1 & 22 & 60222 & 48 \\
    Tomcat Snoop Access & 1 & 24 & 58,122 & 37 \\
    \bottomrule
  \end{tabular}
\end{table}
...
\begin{table}[h!]
  ...
  \begin{tabular}{lrrrrrr}
    ...
    \midrule
    %%Apache Struts 2 & - & - & - & - & - & -\\
    ColdFusion & 4 & 4 & 2\cellcolor{lightgray}$\downarrow$ & 15$\uparrow$ & 5$\uparrow$ \\
    iisadmin access & 1  & 1 & 1 & 1 & 14$\uparrow$\\        
    JBoss JMX & 87 & 87 & 15\cellcolor{lightgray}$\downarrow$ & 84 & 269$\uparrow$ \\
    scripttag & 1 & 1 & 3$\uparrow$ & 1 & 15$\uparrow$ \\
    WeBid LFI & 17 & 17 & 144$\uparrow$ & 27$\uparrow$ & 41$\uparrow$ \\    
    %%Wordpress Login & 8 & - & - & - & - \\
    .htaccess access & 1  & 1 & 1 & 1 & 11$\uparrow$\\
    .idq access & 2  & 2 & 1 & 1 & 15$\uparrow$ \\
    /system32/ in URI & 2  & 2 & 6$\uparrow$ & 1\cellcolor{lightgray}$\downarrow$ & 15$\uparrow$\\
    Basebuilder LFI & 48  & 48 & 846$\uparrow$ & 65$\uparrow$ & 664$\uparrow$\\
    Tomcat Snoop Access & 37  & 37 & 1797$\uparrow$ & 6\cellcolor{lightgray}$\downarrow$ & 3580$\uparrow$\\
    \bottomrule
  \end{tabular}
\end{table}
...
%%  }  
\subsubsection{Running time}\Mar{}
...
with different characteristics. Unfortunately, we could not access
tools for related techniques. For that reason, we made a
side-comparison based on the common examples and results reported in
the original paper. Considering internal validity, the implementation
of \tname\ could contain errors. We mitigated that threat by carefully
inspecting code and results. Considering construct validity, the
choice of ranking to evaluate the approach may not be useful in
practice. We considered the use of ranking of the golden rule a
reasonable choice as the approach places the golden rule close to the
top in most cases.
...
generate false positives w.r.t. the input traffic. \Gui{Fallahi and colleagues proposes a flow-based solution for automated rule generation. Instead of analyzing the packets individually, their algorithm investigates and creates the rules for flows in the network. The algorithm was tested using the ISCX 2012 dataset and showed good results in terms of precision, but it is limited to the type of attack. As it won't analyse individual details of each packet, it wouldn't be able to identify and generate rules for attacks that require detailed options, such as the HTTP PESC, RCE and LFI. https://ieeexplore.ieee.org/abstract/document/7585840}
...
be used for repair. \Gui{Ganesan and colleagues introduces an idea of generating new sets of rules based on modifications of existing rules. They used the community edition of snort rules from 2012 for the modifications and tested with the the pcap file from the MACCDC 2012 dataset. It was reported an increase in the number of alerts, but it wasn't analysed the false alarmrates for abduced rules. Their technique is parallel to the rule creation strategies, as the objective of the modified rules is to capture variations and evolutions of attacks. https://arxiv.org/abs/1903.12101 \Fix{I'm not sure if we can consider it 'rule repair', but I don't know where to put it.}}
