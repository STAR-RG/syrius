...
\numproto\ distinct protocols. Of these total of options,
\totoptionsrelevant\ are relevant to capture traffic (see
Figure~\ref{fig:distribution-rules-protocol}). Table~\ref{table:rules}
...
\begin{table}[h]
  ...
  \centering
  \begin{tabular}{p{0.5cm}lrp{8cm}}
    \toprule
    \multicolumn{1}{c}{\#} & \multicolumn{1}{c}{Name} &
    \multicolumn{1}{c}{\# Params} &  \multicolumn{1}{c}{Description}\\
    \midrule     
...
options that makes a rule to capture the malicious traffic, but
ignores the benign traffic.


\tname{} synthesizes rules with the action header fixed to
\CodeIn{alert} and with the header \CodeIn{<proto> any any -> any
  any}, indicating that it infers the target protocol and uses the
most general flow description possible, \ie{}, the rule applies to the
traffic flowing from/to any address and port. The rule that \tname{}
synthesizes for the SYN Flood denial-of-service attack is as follows.
