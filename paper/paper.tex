...
\newcommand{\textRQtwo}{What is the importance of heuristic functions
  on \tname's performance?}
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,label={}]
...
\newcommand{\textRQthree}{What is the importance of the benign and
  malicious traffic on \tname's performance?}
\vspace{0.2cm}
\begin{itemize}[leftmargin=*,label={}]
...
\begin{table}[h!]
  ...
  \begin{tabular}{lrrr}
    ...
    %%   WeBid & 23 & - & -\\    
    ColdFusion & 19 & 39 & 5\\
    .htaccess access & 17 & 2,458 & 2\\
    .idq access & 19 & 1,111 & 2 \\
    iisadmin access & 17 & 861 & 1 \\
    /system32/ in URI & 20 & 3,665 & 2 \\
...
\subsubsection{Malicious traffic.}To reproduce the malicious traffic
for http and tcp attacks, we configured an http server (Apache2) in
one machine and Nikto2~\cite{nikto} in a different machine. Nikto is
an open source tool with a catalog of scripts to reproduce
vulnerabilities. It is important to note that this setup is \emph{not}
necessary if \emph{pcap} files---pcap is the input format used by
\tname{}---for the attack already exist. As explained in
Section~\ref{sec:minimization}, pcap files abstract network
messages~\cite{pcap} and can be used to avoid network traffic during
rule synthesis. We proceeded as follows to obtain pcap files for the
the attacks we analyzed: we 1)~instantiated the script (\eg{}, defined
the IP of the web server), 2)~executed the Nikto2 script, 3)~and
monitored the traffic with the Wireshark network
monitor~\cite{wireshark-net-monitor}. The Wireshark tool records the
traffic and reports pcap files on output for replaying. The only
difference in the setup of non-http non-tcp attack is in the first
step of this procedure. For example, we used the hping3~\cite{hping3}
command to reproduce the ICMP SYN Flood attack: \CodeIn{hping3 -d 80
  -w 512 -S -p 80 --flood --rand-source <IP>}. This command alone
reproduces a flooding attack on a given machine
(Section~\ref{sec:dos}).\Mar{what about the variants? where do they
  come from? this is confusing as the narrative above suggests the
  attack is hardcoded in the tool.}\Luc{O trafego de um ataque 
  normalmente possui varios pacotes, entre esses varios pacotes alguns 
  deles possuem um padrao. As regras sao criadas descrevendo esse padrao,
  com isso um unico ataque pode disparar varios alertas.
...
%% On each execution, we captured
%% the incoming traffic to the web server with
%% Wireshark~\cite{wireshark-net-monitor} and used the corresponding pcap
%% files as an input to

%% \tname.\Mar{@Lucas, eu traduzi o que vc. falou
%%   para ingles. Mas, aparentemente, isto esta **inconsistente** com o
%%   que eh dito na Secao~\ref{attack-reproduction}. Por favor,
%%   esclareca!}  \Luc{Essa descrição é referente aos ataques HTTP/TCP da
%%   tabela. O hping3 gera apenas o trafego do SYN Flood. Ambas as
%%   explicações estão corretas. Creio q a descrição da seção 5.2.1 tenha
%%   q ser movida para cá pois está explicando como capturamos o trafego
%%   malicioso. Nós executamos o Nikto e o hping3 uma unica vez para
%%   capturar com o wireshark o trafego q eles geram. O trafego produzido
%%   pelo Nikto ja inclui todos os ataques que utilizamos, nós separamos
%%   os pacotes de cada um deles manualmente utilizando o
...
%%   precisa responder por aqui. Mova o texto deixando-o *consistente*.}
...
%available on the web to replay the attacks. For example, 
