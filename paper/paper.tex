...
reported that rule within the top ten rules of the ranking in
...
\emph{ \textbf{Summary of results:}~\tname\ successfully synthesizes
  ...
  ten rules in \percTopFiveRanking\ of the cases.  }
...
consider rules with those options.\Gui{I think that it's not good to say we are the only techinique to support content options. The other papers do not explain well how they support it, if they support for http (which is the most relevant one) or even if they support it at all. \cite{Kao2015AutomaticNR} manages contents in a similar way to ours, but it requires a manual step and their provided email doesn't even exist anymore. \cite{vollmer-etal-cics2011} and \cite{guruprasad} do not support http and don't explain well how they deal with ICMP contents. \cite{fallahi} technique deals with flows and not individual packets, so it doesn't even mention supporting contents.}
