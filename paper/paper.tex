...
options. Furthermnore, we observed that many of these rules indicate
what parts of the message the content string should be checked
against.\Mar{Lucas, (1) vc. tem dados para dar suporte a isto (de http
  e tcp)? ou seja, quantas options do tipo ``content'' possuem
  modificadores.}  For example, the creator of the rule may want to
indicate that a specific string be present in the header of an HTTP
request message. Content \emph{modifiers} serve this purpose. For
example, the modifier \CodeIn{http\_header} in the option
\CodeIn{content: ``keep-alive''; http\_header} states the requirement that
the string ``keep-alive'' be present in the header of the message. (A
keep-alive connection attribute indicates that a single TCP connection
should remain open for multiple HTTP requests/responses.)
Figure~\Fix{adicione pequena figura...verifique se ainda faz sentido
  depois do que falei acima...}

Suricata supports content modifiers for tcp and http. Given that
content options often use modifiers\Mar{assumindo que eh
  verdade. confirmar estat. solicitada acima...}, \tname\ uses
associated frequency to discriminate more or less likely rules. For
example, historical data indicates that content options that use the
\CodeIn{http\_connection} modifiers are much less likely than content
options that use the modifier
\CodeIn{http\_uri}. Figure~\ref{fig:distribution-content_modifiers}
shows the distribution of content modifiers for http. We ommit the
distribution for tcp for space. Similarly to the the previous
heuristic functions, this function uses the frequency of content
modifiers to calibrate the contribution of each rule.
...
A avaliação que fazemos aqui pontua um dado content de acordo com quantas vezes o campo em que ele se encontra eh referenciado nas regras. Por exemplo, vamos supor que o tokenizer produziu a opção content: "/HtmlAdaptor". Essa string pertence ao campo "Request URI:". Esse campo pode ser acessado pelo modificador http\_uri. O tokenizer associa o content a esse modificador. Posteriormente, esse content vai receber uma pontuação correspondente a popularidade desse modificador:\\
\[\frac{10111}{10111*1}\]
A multiplicação por 1 no denominador eh referente a quantidade de modificadores que estão associadas a esse content.
No caso da opção content: "keep-alive", essa string pertence ao campo "Connection:". Como falado acima, o suricata pode acessar esse campo pelos modificadores http\_header ou http\_connection. Portanto, esse content será pontuado de acordo com a popularidade desses dois modificadores:
...
More precisely, we calculate the contribution of the frequency of
content modifiers with the following formula:

\[\frac{\sum_{i=1}^{N}\sum_{j=1}^{M}\frac{modifier\_frequency[modifier_{ij}]}{max(modifier\_frequency)}}{N}\]

\noindent
, where $N$ is the number of contents in a given rule and $M$
is the number of content modifiers\Mar{in a rule? what is a content
  modifier in a rule?}. \Luc{M eh o numero de modificadores associados
  a cada content. Temos content[1], content[2], ..., content[N]. Cada
  um desses contents tem modificadores associados content[i][1],
  content[i][2], ..., content[i][M].}
