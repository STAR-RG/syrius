...
Snort and Suricata. \tname{} synthesizes rules in two steps. First, it
produces a potentially overspecified rule that captures the negative
example provided as input. Then, it uses positive examples to
eliminate unecessary options from the rule. The goal is to create a
rule that captures only the negative traffic.

\subsection{Problem Characterization}

\tname{} uses an abstract representations of a Suricata rule that
reflects the concrete representation of the rule. The concrete rule
representation is the one actually used by Suricata whereas the
abstract representation is the one \tname{} uses to run the
search. The abstract representation encodes the rule options segment
(see Section~\ref{sec:example-suricata-rules}) of a Suricata rule as a
