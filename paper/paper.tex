...
\ie{}, it may be more restrictive than the golden rule (see
Section~\ref{sec:suri-metas-coverage}). Consequently, the use of that
rule could lead to false negatives. Second, \tname{} generates rules
that weakens the constraints expressed in the inferred seed rule
whilest still capturing the malicious traffic. Given that every option
declared in a rule needs to be satisfied for a rule to capture an
attack, any subset of the set of options---inferred in the first
step---captures the malicious traffic by definition. \tname{}
leverages that observation to search for rules containing a proper
subset of rules defined in the original seed option. Although that
approach captures the malicious traffic, it can result in false
positives. Intuitively, there are no restrictions on to what extent a
rule can be relaxed. \tname{} uses benign traffic for that
purpose. The minimization step uses a population-based gradient
descent-like search to find all plausible rules derived from the seed
rule, obtained in the previous step. Finally, \tname{} uses several
heuristic functions derived from a data set of existing rules to rank
the plausible rules generated in the previous step.
...
\begin{table}[h!]
  ...
  \centering
  \begin{tabular}{lcrp{9cm}}
    \toprule
    \multicolumn{1}{c}{Name} &
    \multicolumn{1}{c}{\#~Options Seed Rule} &
