...
\Luc{entrada: conjunto de 20 pacotes SYN que representam o flood.
tempo para gerar a regra: ~1m30s
no caso do syn flood, geramos a mesma regra.
}
...
action associated with the rule. \Luc{Para opções cujos valores são numéricos, o suricata apenas verifica se o valor da opção no pacote é igual/maior/menor ao valor especificado na regra, retornando um booleano indicando isso. Alteramos o codigo fonte para obter a diferença entre o valor da opção no pacote e o valor da opção na regra.}
...
\begin{table*}[t!]
  ...
  \begin{tabular}{lllll}
    ...
    \midrule
    GET/POST Flood & multiple & HTTP & Denial of Service & - \\      
    Ping Scan & single & ICMP & Discovers active hosts in a network &  - \\
    Black Nurse & multiple & ICMP & Denial of Service & - \\
    Ping Flood  & multiple & ICMP & Denial of Service & - \\  
    Port Scan TCP & single & TCP & Discovers open ports in a host & -  \\
    SYN Flood & multiple & TCP & Denial of Service & - \\
    Port Scan UDP & single & UDP & Discovers open ports in a host & - \\        
    \bottomrule
  \end{tabular}
\end{table*}
