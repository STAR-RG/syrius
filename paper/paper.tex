...
\begin{table*}[t!]
  ...
  \begin{tabular}{llllp{10cm}}
    ...
    SYN Flood & hping3 & DoS & TCP & \textbf{flags}: S,12;
    ...
    5; \\
    Black Nurse & NetReseC & DoS & ICMP & \textbf{icode}:3; \textbf{itype}:3;\textbf{threshold}: type both, track by\_dst, count 250, seconds
    1; \\
    \midrule
    ...
    WeBid \MyComment{Local File Inclusion} & RFI & LFI & HTTP & \textbf{content}:"GET "; depth:4; \textbf{content}:"/cron.php?"; http\_uri; nocase; \textbf{content}:"include\_path="; http\_uri; nocase; \textbf{content}:"../"; \\
    ...
    Apache Struts 2 & WRCCDC & RCE & HTTP & \textbf{content}:"POST"; http\_method; \textbf{content}:"java.lang.ProcessBuilder"; nocase; http\_client\_body; fast\_pattern; \textbf{content}:"/struts2-rest-showcase/orders/3"; http\_uri; \\
    WordPress Login & WRCCDC & PVE & HTTP & \textbf{content}:"log="; http\_client\_body; \textbf{content}:"\&pwd="; http\_client\_body; \textbf{content}:"\&wp-submit="; http\_client\_body; \\
    \bottomrule
  \end{tabular}
\end{table*}
...
\begin{table}[h!]
  ...
  \begin{tabular}{lrrrr}
    ...
    UDP Flood & - & - & - & - \\
    Black Nurse & - & - & - & - \\
    JBoss JMX & 5 & 23 & 228 & \Fix{23} \\
    WeBid & 1 & 23 & - & -\\    
    ColdFusion & 7 & 19 & 34 & \Fix{5}\\
    .htaccess access & 5 & 17 & 984 & \Fix{2}\\
    .idq access & 14 & 19 & 511 & \Fix{2} \\
    iisadmin access & 4 & 17 & \Fix{861} & \Fix{1} \\
    /system32/ in URI & 4 & 20 & 1,913 & \Fix{2} \\
    Script tag in URI & 23 & 22 & 786 & - \\
    Apache Struts 2 & - & - & - & - \\
...
of attacks we analyzed. Column ``Name'' shows the name of the attack,
column ``Source'' shows the source where we obtained data to reproduce
the attack (see Section~\ref{subsec:malicious-traffic}), column
``Type'' shows the kind of attack, column ``Protocol'' shows the name
of the protocol explored in the attack, and column ``Ground Truth''
shows the rule options to capture the attack. The rationale for the
selection were (1) to cover attacks with different characteristics,
(2) to cover attacks of various protocols, and (3) to explore
different features of \tname. As for (1), the selection includes
attacks of the following types: Denial-of-Service
(DoS)~\cite{denial-of-service}, Active Reconnaissance
(AREC)~\cite{active-reconnaissance}, Privilege escalation
(PVE)~\cite{privilege-escalation}, Remote Code Execution
(RCE)~\cite{remote-code-execution}, Local File Inclusion
(LFI)~\cite{local-file-inclusion}, and Policy Violation
(PV)~\Fix{local-file-inclusion}. Note that the distribution of attacks
is not uniform across protocols as these distributions are not uniform
in practice---http attacks are prevalent.
...
NetReseC~\cite{pcap-attacks}\Fix{confirm}).\Mar{reminder: we still
  ...
  e WordPress. @Guilherme adicionar fontes} \Gui{O NetReseC possui uma lista de sites (alem dos pcaps que o proprio site ja oferece, como foi obtido o Black Nurse) que oferecem datasets, como o proprio Stratosphere, que estamos utilizando para trafego normal. Para esses dois ataques novos com content, peguei do WRCCDC - Western Regional Collegiate Cyber Defense Competition: https://archive.wrccdc.org/pcaps/}In addition to
