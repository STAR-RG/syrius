...
search.\Mar{Technically, there are $32 (=2^5)$ subsets for a set with 5
  elements. So, it is important to indicate why Table 3 shows sooo
  many plausible rules and that this is is a very simple rule (for illustration).}
...
protocol. Figure~\ref{fig:distribution-contents} shows the
distribution of the number of content options for the database of
rules we used\Mar{reference} \Luc{essa distribuicao n eh utilizada 
para pontuar as regras desse exemplo, n seria mais interessante citar outra?
poderia ser a de tamanhos das regras.}. Note that rules with 1 to 3 options 
are more common \Luc{1 to 3 \textbf{content} options}. At the end of the 
process, \tname{} ranks the golden rule at the top of the ranking 
including \Fix{XXXXXX} other plausible rules.

\Mar{add short par indicating that other problems can be much more
  complex than this...}
