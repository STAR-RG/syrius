...
time-consuming and error-prone \rui{is there a citation we could add here?}. The language for describing rules is
...
scarce as it is not common (and considered unethical~\Mar{look for
  citation for ethics code on this}) to publish attacks on the
web\Mar{is there a term for non-deep web?}. Classical machine learning techniques \emph{are not directly
...
\tname{} takes on input one or more examples \rui{We ought to explain how does one get hold on examples. How diffcult are they to obtain?} of an attack and produces
...
We evaluated \tname\ against \Mar{@Lucas, please look for constant
  numbers as this *in text* and use macros instead. We do not want to
  leave inconsistent data in the paper!}14 attacks of various kinds, such as \Fix{...}\rui{add few examples}. The observed results are encouraging. For 
...
the top ten rules in 11 of the 14 cases. \rui{to discuss, I find 
this a bit weak.}
...
Table~\ref{table:rules} shows a sample of non-documentation options \rui{what 
is the meaning of non-documentation in this context?} \Luc{documentation options
are options that gives information about the rule itself and are not used for 
matching on packet data. So, non-documentation options are the ones that really
matters for detection.} of Suricata. Column ``\#'' shows the option number, column 
``Name'' shows the name of the option, and column ``Description'' summarizes
purpose. The option \CodeIn{content} deserves attention. It is common
...
option \CodeIn{content:''SELECT''} checks if the string ``SELECT'' is
...
\CodeIn{fragbits} rule option. In summary, the value of several
...
Figure~\ref{fig:pingscan-example} \rui{any reason for the bolds?} \Luc{these are
the options that are important for detection, the other ones are documentation
options} shows a Suricata rule that captures
...
takes as input an over-constrained rule $sr$ and pcap files~\cite{pcap}
...
\begin{table*}[t!]
  ...
  \begin{tabular}{lcccp{10cm}}
    ...
    UDP Teardrop & \cite{udp-teardrop-source} & DoS & udp & \textbf{fragbits}:M; \textbf{id}:124; \\
      2100384 & - & ICMP Info & icmp & \textbf{icode}:0; \textbf{itype}:8; \\
      2100399 & - & ICMP Info & icmp & \textbf{icode}:1; \textbf{itype}:3; \\
      2100401 & - & ICMP Info & icmp & \textbf{icode}:0; \textbf{itype}:3; \\
      2100402 & - & ICMP Info & icmp & \textbf{icode}:3; \textbf{itype}:3; \\
      2100408 & - & ICMP Info & icmp & \textbf{icode}:0; \textbf{itype}:0; \\
      2100485 & - & ICMP Info & icmp & \textbf{icode}:13; \textbf{itype}:3; \\
      2100486 & - & ICMP Info & icmp & \textbf{icode}:10; \textbf{itype}:3; \\
