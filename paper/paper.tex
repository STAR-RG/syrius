...
\tname{} produced a total of 23 tokens\Mar{@Lucas, isto estah certo?
  Se sim, como isto cai para 5 (pela Tabela 3)} \Luc{6.1.1 eh o ataque 
JBoss. Na tabela constam 23 tokens.}, considered content
option candidates.
...
that is passed as parameter\Mar{isto realmente eh necessario? se sim,
  ...
  log do \suri} \Luc{Nao precisa desse ponto. Para essa etapa foi preciso modificar o codigo do suricata para q ele fosse capaz de gerar mais de 15 alertas para um unico pacote. Durante a execução do Syrius pode ser gerada uma grande quantidade de regras q causam falsos positivos, eventualmente podem haver 15 delas q detectam erroneamente um mesmo pacote. Sem essa alteração, algumas regras q geram falso positivo n seriam eliminadas.}, 2)~replaying the negative benign traffic (see
...
was the best we found. \Mar{$\leftarrow$indique aqui quais foram os
  pesos que a gente usar para cada funcao.} \Luc{f1: 1, f2: 0.25, f3: 0.25, f4: 1} Although empirical results
...
\newcommand{\percTopFiveRanking}{66.7\% (=10/15)}
