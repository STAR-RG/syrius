...
for the heuristic functions. The ranking of a golden rule, for a set of $N$ 
attacks, is given by $R(W) = \sum_{i=1}^{N}\mathit{R_i}$, where 
$R_i = (w_1 \cdot popt) \cdot (w_2 \cdot ncont) \cdot (w_3 \cdot pcont) \cdot (w_4 \cdot phttp)$
and $W = \langle{}w_1, w_2, w_3, w_4\rangle{}$ are the weights considered for each heuristic.
%
