...
The first step of \tname\ is Reverse Engineering. The goal of this
step is to extract options from the malicious traffic. A rule that
contains these options, by construction, captures the malicious
traffic. \tname\ uses two strategies to extract options: 1)~field and
2)~payload. We elaborate these strategies in the following.
...
constructed the mapping of network fields to options manually, from
the \suri\ documentation.

Unfortunately, the relevant data for the option may be at the message
payload not the fields. Figure~\ref{fig:rule-jboss} shows an example
of a rule that captures traffic containing the string
\CodeIn{"/HtmlAdaptor"} in the payload.  The content strategy focuses
on extracting (content) options from the payload. It is very important
to note that content options are prevalent. For example,
\percRulesWithContent{} of the \numrulessuri{} rules from the public
data set~\cite{emerging-threats-open} we analyzed contain at least one
option of this kind. Figure~\ref{fig:distribution-contents} shows the
histogram of the number of these options per rule for the data set
mentioned above. We limited the number of content options per rule to
10 for space. Rules without \CodeIn{content} options are an exception
and half of the rules contain at least two \CodeIn{content} options.
...
spaces. We observed, by inspecting existing rules, that
\CodeIn{content} options typically refer to sequence of characters in
the payload of messages separated by these delimiters. This
tokenization step can generate dozens of options.  For example,
considering the example from Section~\ref{sec:content-example},
\tname{} produces a total of \Fix{XXX} content options.
...
\textbf{Implementation.}\Fix{elaborate...}
...
\begin{table}[h!]
  ...
  \begin{tabular}{lrrr}
    ...
    WeBid & 23 & - & -\\    
    ColdFusion & 19 & - & 5\\
    .htaccess access & 17 & - & 2\\
    .idq access & 19 & - & 2 \\
    iisadmin access & 17 & - & 1 \\
    /system32/ in URI & 20 & - & 2 \\
