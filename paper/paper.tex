...
\begin{table}[t!]
  ...
  \begin{tabular}{lrrrr}
    ...
    /system32/ in URI & 4 & 20 & 1,881 & 2 \\
    Basebuilder LFI & 1 & 22 & 60,222 & 48 \\
...
\begin{table*}[t!]
  \footnotesize
  \caption{\label{table:attacks}-}
  \vspace{-2ex}
  \centering
    \begin{tabular}{lp{2cm}p{2.1cm}ccp{2cm}c}
    \toprule
    ...
    \midrule
    \cite{vollmer-etal-cics2011} & Malign Traffic & Manually crafted & ICMP & Rule List & TP, FP & No\\
    \midrule
    \cite{fallahi} & Mixed Traffic & \cite{iscx} & HTTP, SSH, TCP & Single Rule & TP, FP, Precision, Recall, F1 & No\\    
    \midrule
    \cite{Kao2015AutomaticNR} & Malign Traffic, Benign Traffic, Rule Dataset & Manually crafted, \cite{emerging-threats-open} & HTTP & Single Rule & Generating time, FP & No \\
    \midrule
    \cite{guruprasad}& Malign Traffic & \cite{iscx} \cite{darpa1999} & ICMP, TCP & Rule List & FP & No\\
    \midrule
    Syrius & Malign Traffic, Benign Traffic, Rule Dataset & Manually crafted, \cite{emerging-threats-open}, \cite{iscx} & HTTP, TCP, ICMP & Rule List & Rule Ranking & Yes \\
    \bottomrule
  \end{tabular}
\end{table*}
...
%\vspace{-2.5ex}  
\caption{Impact of number of examples.}
...
%%  }  
\subsubsection{Running time}\Mar{}\Gui{We ran all our tests using an Intel(R) Xeon(R) CPU E3-1240 v6 and 8GB RAM. For attacks with only categorical and numerical options, the average running time was 6 seconds per attack. For attacks with the content option, the average running time was 1445 seconds.}
