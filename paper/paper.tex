...
\Fix{tabulate some important rules that we support}
...
maximal if no other option can be added without making it miss the
traffic.
...
triggering rule actions. This plot indicates that the problem search
space is defined based on the protocol of the malicious traffic. For
instance, if \tname{} detects that the attack involves \CodeIn{http}
...
level is insufficient to capture content options (see
...
contain content options. Figure~\ref{fig:distribution-contents} shows
the histogram of number of content options per rule (up to 10) for the
Suricata public rule database \Fix{cite}. Rules without content
...
\begin{wrapfigure}[12]{r}{0.5\textwidth}
  ...
  \caption{\label{fig:distribution-contents}Histogram of number of
    content options per rule (up to 10).}
\end{wrapfigure}
...
inspecting existing rules, that content options typically refer to
...
Any rule created from a subset of rule options still captures the
traffic as every rule option we infer in this step is independent and
satisfied by the traffic.
