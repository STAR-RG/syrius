...
\tname{} uses the size of existing rules as a heuristic function. For
example, we noted that most rules in the dataset have 2 to 5 options,
so \tname{} values rules in this size range higher compared to other
sizes. Figure~\ref{fig:distribution-rule-size} shows the histogram of
rule sizes. The x-axis shows the number of options (\ie{}, size)
included in a rule and the y-axis shows the number of rules that have
that number of options. \tname{} uses this information to rank rules.
...
Regras com um tamanho mais comum pontuam mais.
Nesse caso, a pontuação dada por esse componente será: \[\frac{rules\_per\_size[len(rule)]}{max(rules\_per\_size)}\]
