...
fake IP addresses. Then, after the server responds with a SYN-ACK
packet, the client sends another SYN to avoid the connection to time
out. Without proper protection, the server accepts the connection
requests and eventually legitimate connection requests cannot be
satisfied due to resource exhaustion.
...
of two components, each implementing one of the steps mentioned above.
The goal of the first component is to produce a rule that captures the
negative traffic.  First, it initializes the rule encoding with random
values. Only options associated with the used protocol are included in
the encoding. Then, \tname\ optimizes the rule options until \suri\ is
able to capture the attack.  Note that, as all options need to be
satisfied for the traffic to be captured, at this point, any subset of
options would capture the negative traffic. The second component of
the pipeline is initialized with the rule produced in the previous
step. That rule captures the negative traffic but could also capture
benign traffic. This component systematically discards options from
the rule encoding until no positive traffic is captured.

\Fix{describes negative and positive traffic}
