...
$(w_1 + popt) \cdot (w_2 + ncont) \cdot (w_3 + pcont) \cdot (w_4 + phttp)$ 
and $W = \langle{}w_1, w_2, w_3, w_4\rangle{}$ are the weights considered 
for each heuristic.
%
We resorted to the Differential Evolution algorithm from SciPy\cite{diff-evo-scipy}
to compute the optimal weights. The optmization consists in minimizing the function 
of every attack's golden rule ranking in our set given a combination of weights. 
