...
analyzing network messages directly. Instead of sending traffic to a
machine with a deployed server (\eg{}, web server) that
\suri\ monitors, our setup calls Suricata directly through an
API. Suricata is invoked multiple times in the process of minimizing
the rule.
\textbf{Limitations.} It is worth noting that although
...
requires one sample, but can benefit of several samples. The same
applies for the negative traffic. In this case, however, \tname{} can
use public data sets available online.
...
\noindent
\textbf{Rationale.}~The golden rule is a trusted human-written rule
obtained from credible sources. \tname\ generates several rules that
satisfy the invariant to only capture the malicious traffic, but the
amount of data we use to evaluate that property, albeit large, is
limited. Assessing the quality of each rule \tname\ produces (\ie{},
fitness to the attack) requires domain knowledge and much effort. The
position of the golden rule at the ranking produced by \tname\ is a
proxy measure to the effort a system administrator would have in
finding the rule that fits the purpose.
...
\noindent
\textbf{Rationale.}~\tname\ proposes a variety of heuristic functions
to rank the plausible rules produced during minimization. It is
therefore important to understand how much each function contributes
to the overall performance of the technique.


\newcommand{\textRQthree}{What is the importance of the positive and
  negative traffic in ranking?}
\vspace{0.2cm}
\begin{itemize}[leftmargin=*]
...
technique is to the traffic that it uses.
\noindent
\vspace{1ex}

The following sections describe the objects we used to evaluate the
technique (Section~\ref{sec:dataset-benign}), answer the questions
(sections~\ref{sec:answer-rqone},~\ref{sec:answer-rqothree}), and
...
\label{sec:answer-rqotwo}

\subsection{Answering RQ3: \textRQthree}
\label{sec:answer-rqothree}
