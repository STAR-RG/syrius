...
ranked the correct rule among the first ten rules of the reported
...
\percTopFiveRanking\ of the cases. \rui{to discuss, I find 
...
%\vspace{-1ex}
\begin{figure}[h!]
  \lstinputlisting[columns=flexible, language=C,numbers=none,keywords={dsize,itype}]{pingscan.suricata}
  \vspace{-1ex}  
...
option. Figure~\ref{fig:adaptor-golden-rule.suricata} shows an example of a rule that
...
rules $N$ times \rui{is N an input? How does one determine it in practice?} \Gui{Yes. Higher N values increases the chances of creating every possible rule for the attack (decreasing the negative effects of the non-determinism), at the same time that increases the computational cost. So the value can be increased in case the desired rule is not being created, or it can be reduced if the code is creating too many rules. In our code, we chose manually N=2, as it was already enough for showing the desired results.}, discarding one rule option each
...
weights. \Gui{We used Differential Evolution algorithm from SciPy to find the best weights for our set of attacks. The optmization for our weights consists of minimizing the function that returns the sum of every attack's golden rule ranking in our set given a combination of weights. 
\indent
$\sum_{i=1}^{N}\mathit{R_i}$ ($\mathit{R_i}$, where $\mathit{R}$ is the ranking of a golden rule and $\mathit{N}$ is the number of attacks)
The algorithm also receives as input the range which the weights can change (between 0 and 1) and the max number of generations, which we set to 10 (setting higher values could provide better results, but also would take more time). At the end, the optimization returned the weights [0.28058353, 0.05713404, 0.043285, 0.49022223]}
...
%We discuss in the following techniques similar in goal to ours. Unfortunately
