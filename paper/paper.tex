...
common. \Luc{Se for preciso temos numeros para fundamentar isso
  ...
  (\%) das 28K regras tem contents.} As consequence, we defined a
...
should be checked against.\Mar{Lucas, (1) vc. tem dados para dar
  suporte a isto (de http e tcp)? ou seja, quantas options do tipo
  ``content'' possuem modificadores.} \Luc{conseguirei esses dados}
For example, the creator of the rule may want to indicate that a
specific string be present in the header of an HTTP request
message. Content \emph{modifiers} serve this purpose. For example, the
modifier \CodeIn{http\_header} in the option \CodeIn{content:
  ``keep-alive''; http\_header} states the requirement that the string
``keep-alive'' be present in the header of the message. (A keep-alive
connection attribute indicates that a single TCP connection should
remain open for multiple HTTP requests/responses.)
Figure~\Fix{adicione pequena figura...verifique se ainda faz sentido
  depois do que falei acima...}
...
\tname\ uses the frequency of content modifiers to discriminate more
or less likely rules. For example, historical data indicates that
content options that use the \CodeIn{http\_connection} modifier are
...
modifiers are applicable for rules of any protocol in Suricata,
currently, this heuristic function is only supported for tcp and http
in \tname. The rationale is that (1) these are the protocols involving
most attacks \Luc{Ainda n encontrei nenhuma informação concreta afirmando isso. N podemos citar o fato de que 80\% das regras do dataset são desses 2 protocolos?}\Mar{Lucas, please confirm.} and (2) the payload of
messages of different protocols are not as segmented as the case of
tcp and http, which makes the reference to specific parts of the
payload less common for rule writers and more challenging for
\tname\Mar{Lucas, please confirm}\Luc{verificar}.

Similarly to the the previous heuristic functions, this function uses
the frequency of content modifiers to calibrate the contribution of
each rule. More precisely, we calculate this heuristic function with
the following formula:

$\sum_{i=1}^{N}\frac{\sum_{j=1}^{M_i}\frac{\mathit{modifier\_frequency[modifier_{ij}}]}{\mathit{max(modifier\_frequency)}}}{M_i}$

\noindent
, where $N$ is the number of contents in a given rule and $M$ is the
number of content modifiers\Luc{$M_i$ eh o numero de modificadores que podem referenciar a posição em q se encontra o $content_i$}\Mar{isto eh confuso. vc. esta dizendo que
...
\begin{figure}[H]
  ...
  \caption{\label{fig:distribution-content_modifiers}Content modifiers
    frequency.\Mar{what about ``none'' (no modifier)?}}
\end{figure}
...
Consider a rule containing the option \CodeIn{content:
  ``/HtmlAdaptor''; http\_uri}, as the one from
Section~\ref{sec:jboss}. The contribution of this option to the score
of the corresponding rule would be $\frac{10,111}{10,111*1}$. The
value of $N$ for this case is 1 as the field of the HTTP message
\CodeIn{Request URI} holding the string can be accessed only through
the modifier \CodeIn{http\_uri}.
...
modificadores que estão associadas a esse content.\Luc{O correto eh ser dividido pela quantidade de modificadores. Atualizei a formula para tentar representar isso. Considere que no exemplo acima o content pudesse ser acessado por mais de um modificador. A pontuação dele então seria $\frac{10,111+x}{10,111*1}$ > 1. Para manter a pontuação entre 0-1, o certo nesse caso seria fazer $\frac{10,111+x}{10,111*2}$}\Mar{nao foi isto
  ...
  contents e o exemplo acima tem apenas 1 content.}  No caso da opção
content: "keep-alive", essa string pertence ao campo
"Connection:". Como falado acima, o suricata pode acessar esse campo
pelos modificadores http\_header ou http\_connection. Portanto, esse
content será pontuado de acordo com a popularidade desses dois
modificadores:
\[\frac{4089+29}{10111*2}\]
...
\vspace{1ex}
\noindent
\textbf{Malicious traffic.}~Nikito?\Fix{where do these
