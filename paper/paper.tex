...
%        \legend{relevant, irrelevant}
  ...
  \vspace{-2ex}
  \caption{\label{fig:distribution-rules-protocol}Number of rules per number of contents.}
%\end{wrapfigure}
\end{figure}

}

\Mar{Lucas, mova isto para secao ``Ranking'' da tecnica. Aqui
  vc. explica em alto-nivel como isto funciona. Ha alguma funcao
  heuristica que afeta este exemplo?}
 \Fix{...informa um
  exemplo concreto que vc. usaram...}. The list below shows the rule
...
fields. Handling this case is critically important as \Fix{xx}\% of the
rules contain \CodeIn{content} options as the one from
...
\Mar{pessoal, isto explica como inferimos opcoes de content, mas nao
  explica como inferimos as outras opcoes. Em particular, as opcoes
  dos primeiros dois exemplos da Secao 3.}
\Luc{as outras opções, em geral, possuem um campo correspondente no pacote. isso ja esta sendo dito na linha 837/838 e é explicado na subseção 2.3, n?}
