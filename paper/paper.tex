...
fake IP addresses. \Luc{Then, after the server responds with the usual SYN-ACK packet, the client sends another SYN to avoid the connection time out.} \Mar{Are you saying that there is no timer that the
  server starts when it sends a SYN-ACK message?} Without proper protection, the server accepts the
