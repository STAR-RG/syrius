...
vulnerabilities. This methods is used as a preliminary step towards an
actual attack. For example, Ping Scan scans the network for IPs of
...
these requests. This method enables an attacker to know which machines
...
Figure 4 shows a Suricata rule that captures a Ping Scan attempt, as
documented in NMAP~\cite{netmap}, a network discovery and security
auditing open-source tool. The relevant options are in bold---the
...
any address and port.  The choice of these parameters are
...
benign traffic to pass.

\tname{} proceeds as follows to synthesize rules. First, it analyzes the malicious
traffic and extracts options expressed in that traffic. Recall from
Section~\ref{sec:rules-and-packets} that rule options can be inferred
from packet contents. \tname{} finds the following set of options at
this stage:
...
malicious traffic. Unfortunately, the rule above is
...
in false negatives. In this case, although options \CodeIn{dsize:0}
and \CodeIn{itype:8}~(see Figure~\ref{fig:pingscan-example}) are
present in the set of inferred options, the others are not.
...
produces a total of \Fix{XXXXX} plausible rules at the end of the
search.
...
example, one heuristic function used by \tname{} leverages the amount
of options a rule typically has. The score of a rule, given by this
function, is proportional to the popularity of the number of options
declared in rules of a given protocol. At the end of the process,
\tname{} ranks the golden rule at the top of the ranking including
\Fix{XXXXXX} other plausible rules.


%% the following rules 
%% at the top of the ranking produced by \tname{}:
%% 1)~{\small\CodeIn{itype:8; dsize:0;}}, 2)~{\small\CodeIn{itype:8;
%%     icmp\_id:23570;}}, 3)~{\small\CodeIn{icode:8; icmp\_id:23570;}},
%% 4)~{\small\CodeIn{itype:8; icode:0; dsize:0;}},
%% 5)~{\small\CodeIn{icmp\_id:23570}}. Note that the golden rule appears
%% at the top position.
...
first step to create a potentially overspecified seed rule, it uses
the benign traffic in the second step to create a set of plausible
rules from the seed rule, and it uses public databases with existing
rules in the third step to rank plausible rules.
...
%% \Mar{...and where did they come from?} \Luc{Sim, floods podem ser compostos por milhares de pacotes, um dos datasets q encontrei possui mais de 500k pacotes de flood. Os datasets foram encontrados na internet.}
...
rules contain \CodeIn{content} options.
...
%% the dataset of existing rules (see
%% Section~\ref{sec:dataset-benign})---that these rules often indicate
