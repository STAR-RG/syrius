...
\begin{table*}[h!]
  ...
  \begin{tabular}{lll}
    ...
    JBoss JMX Console Beanshell Exploit \MyComment{& single} & HTTP \MyComment{& Remote Code Execution} & \scriptsize{content:"/HtmlAdaptor"; nocase; http\_uri; content:"action=inspect"; nocase; http\_uri; content:"bean"; nocase; http\_uri; content:"name="; http\_uri;} \\
    WeBid Local File Inclusion \MyComment{& single} & TCP \MyComment{& Local File Inclusion} & \scriptsize{content:"GET "; depth:4; content:"/cron.php?"; http\_uri; nocase; content:"include\_path="; http\_uri; nocase; content:"../";} \\
    ColdFusion Privilege Escalation \MyComment{& single} & HTTP \MyComment{& Privilege Escalation} & \scriptsize{content:"GET"; http\_method; nocase; content:"/CFIDE/administrator"; http\_uri; nocase;} \\
    \Luc{Adicionar nome} \MyComment{& single} & HTTP \MyComment{& \Luc{Adicionar}} & \scriptsize{content:".htaccess"; nocase; http\_uri;} \\
    \bottomrule
  \end{tabular}
\end{table*}
...
files as an input to \tname.\Mar{@Lucas, eu traduzi o que vc. falou
  ...
  \Luc{Essa descrição é referente aos ataques HTTP/TCP da tabela. O hping3 gera apenas o trafego do SYN Flood. Ambas as explicações estão corretas. Creio q a descrição da seção 5.2.1 tenha q ser movida para cá pois está explicando como capturamos o trafego malicioso. Nós executamos o Nikto e o hping3 uma unica vez para capturar com o wireshark o trafego q eles geram. O trafego produzido pelo Nikto ja inclui todos os ataques que utilizamos, nós separamos os pacotes de cada um deles manualmente utilizando o wireshark.}
