...
\begin{table*}[t!]
  \small
  \caption{\label{table:attacks}List of attacks.\Mar{adicionar coluna ``Source''}}
  \centering
  \begin{tabular}{llllp{11cm}}
    ...
    \midrule     
    Ping Scan & NMAP & Active Recon. & ICMP & \textbf{dsize}:0; \textbf{itype}:8; \\
    ...
    %% WeBid \MyComment{Local File Inclusion} & RFI & TCP \MyComment{& Local File Inclusion} & \textbf{content}:"GET "; depth:4; \textbf{content}:"/cron.php?"; http\_uri; nocase; \textbf{content}:"include\_path="; http\_uri; nocase; \textbf{content}:"../"; \\
    ColdFusion \MyComment{Privilege Escalation} & Nikto2 & Privilege Escalation & HTTP \MyComment{& Privilege Escalation} & \textbf{content}:"GET"; http\_method; nocase; \textbf{content}:"/CFIDE/administrator"; http\_uri; nocase; \\
    .htaccess access\MyComment{attempted-recon} & Nikto2 & RCE & HTTP \MyComment{& Adicionar} & \textbf{content}:".htaccess"; nocase; http\_uri; \\
    ...
    /system32/ in URI\MyComment{Possible Protected Directory Access Attempt} & Nikto2 & RCE & HTTP \MyComment{& Adicionar} & \textbf{content}:"/system32/"; nocase; http\_uri; \\
...
\begin{table}[h!]
  ...
  \begin{tabular}{lrrrr}
    ...
    \multicolumn{1}{c}{\multirow{2}{*}{\# Variants}} &
    \#~Option Cand. &
    \#~Plausible Rules &    
    ...
    Ping Scan & 5 & 5 & 2 & - \\
    SYN Flood & 3 & 2 & 2 & - \\
    JBoss JMX & 4 & 23 & 381 & 23 \\
    %%   WeBid & - & 23 & - & -\\    
    ColdFusion & 6 & 19 & 39 & 5\\
    .htaccess access & 4 & 17 & 2,458 & 2\\
    .idq access & 13 & 19 & 1,111 & 2 \\
    iisadmin access & 3 & 17 & 861 & 1 \\
    /system32/ in URI & 3 & 20 & 3,665 & 2 \\
    Script tag in URI & 22 & 22 & - & - \\
    \bottomrule
  \end{tabular}
\end{table}
...
\subsubsection{Attacks.}Table~\ref{table:attacks} shows the attacks we considered in our
...
shows the options of the rule that captures the attack.\Mar{Explicar o
  rationale para esta selecao de ataques!}
...
\subsubsection{Malicious traffic.}~For some of the attacks listed on
Table~\ref{table:attacks}, we obtained pcap files directly from
trusted security websites (\eg{},
NetReseC~\cite{pcap-attacks}). Recall that \tname\ takes pcap files on
input. As explained in Section~\ref{sec:minimization}, these files
abstract network messages~\cite{pcap} and can be used to avoid network
traffic during rule synthesis. In addition to these sources, we used
the Nikto2~\cite{nikto} open source security tool to reproduce
attacks. To reproduce the http and tcp attacks listed on
Table~\ref{table:attacks}, we configured an http server (Apache2) in
one machine and ran the security tool in a different
machine. Simultenously, we ran WireShark~\cite{wireshark-net-monitor}
to record the network traffic and Suricata to analyze the traffic. We
configured \suri\ with all rules from the ``Emerging Threats Open
Ruleset''~\cite{emerging-threats-open} and ran Nikto2 with default
parameters, which makes it replay all recorded attacks. After
execution, we inspected the \suri\ log to identify which rules have
been activated and which packets were involved in each
...
indicating that there are multiple instances of the same attack
encoded in Nikto2. RQ3 evaluates the importance of these variants to
\tname{}'s performance. Finally, Nikto2 only supports tcp and http
attacks.  We used the hping3~\cite{hping3} to reproduce the Ping Scan
icmp attack (see Section~\ref{sec:dos}).  More precisely, we used the
following hping3 command to reproduce that attack: \CodeIn{hping3 -d
  80 -w 512 -S -p 80 --flood --rand-source <IP>}.
...
\subsubsection{Benign traffic.}~Security
tools (e.g., perforation testing tools) often use benign traffic to
evaluate false positive rates. Consequently, different public options
exist. We used two popular public sources. The Bigflows.pcap data set
is part of the TcpReplay~\cite{tcpreplay} open source project. This
data set includes real network traffic of a busy private network
access point to the Internet. According to the TcpReplay web site
``This capture is much larger and has a smaller average packet size
than the previous capture (smallFlows.pcap). It also has many more
flows and different applications.''. In addition to the Bigflows.pcap
data set, we also used data sets\Mar{more than one?} made available by
Stratosphere Lab~\cite{stratosphere-normal}\Mar{is there a description
  of the traffic? as the description for Bigflows.pcap?}.
