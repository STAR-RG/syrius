...
rules not previously captured. \tname{} is organized as a pipeline of three components that
...
evidence that \tname\ is promising. \rui{the example is interesting, but this gives the 
feeling that the work is too preliminary} For example, it was
...
to attack a local network. The example rule prescribes a method to
...
often created.  Rulesets need to be continuously revised because the
community finds and advertises vulnerabilities, which eventually can be exploited by attackers. 
...
This paper proposes \tname{} (for \textbf{Sy}nthesis of
...
scarce as it is not common to publish attacks openly on the
...
We evaluated \tname\ against 14 attacks of various kinds, such as 

\rui{add few examples}. The observed results are encouraging. For 
...
Table~\ref{table:rules} shows a sample of non-documentation options \rui{what is the meaning of non-documentation in this context?} of
