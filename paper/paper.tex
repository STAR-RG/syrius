...
should be checked against.\Mar{Lucas, vc. tem dados para dar suporte a
  ...
  modificadores.} \Luc{conseguirei esses dados} For example, the
...
for multiple HTTP requests/responses.)  Figure~\Fix{adicione pequena
  figura...verifique se ainda faz sentido depois do que falei
  acima...}
...
modifiers are applicable to rules of any protocol in Suricata,
currently, \tname\ only supports tcp and http. These are the protocols
involving ~80\% of the attacks.
...
each rule. More precisely, we compute this function with the following
formula:

\[\frac{\sum_{i=1}^{N}\frac{\sum_{j=1}^{M_i}\frac{\mathit{modifier\_frequency[modifier_{ij}}]}{\mathit{max(modifier\_frequency)}}}{M_i}}{N}\]

\noindent
, where $N$ is the number of contents in a given rule and $M_i$ is the
number of content modifiers that could access the string associated
with content $i$. \Mar{Qual o rationale para considerar as varias
...
\begin{figure}[t!]
  ...
  \Luc{Uma regra pode sim ter contents sem modificadores, mas isso n vem ao caso aqui. Nessa heuristica toda string do payload vai ter associada a ela os modificadores q podem acessar a sua posição. Portanto, não vai existir o caso de não haver modificador. Atenção: isso não quer dizer q as regras q geramos incluem os modificadores, elas n incluem.}
\end{figure}
...
of the corresponding rule would be $0.5=\frac{10,111}{10,111*1}$. The
value of $M_i$ for this case is 1 as the field of the HTTP message
\CodeIn{Request URI} holding the string can only be accessed through
the modifier \CodeIn{http\_uri}. For the option \CodeIn{content:
  ``keep-alive''; <mod>}, either \CodeIn{http\_header} or
\CodeIn{http\_connection} could substitute the placeholder
\CodeIn{<mod>} to locate the string ``keep-alive'' stored in the field
\CodeIn{Connection} in the header of an HTTP request
message\Mar{Lucas, a imagem mostrando header com campos Request URI e
  Connection e os conteudos citados acima pode ajudar.}\Luc{adicionar}. Consequently,
for this case, the constribution of content modifiers would be
$0.2=\frac{4089+29}{10111*2}$. Assuming that both options are
mentioned in a rule, the aggregated contribution of this heuristic
function would be \Fix{...}\Mar{Lucas, confirme que nao esta faltando
  calcular a media (dividir tudo por N).} \Luc{Sim, tem q dividir. A implementação estava errada, está de acordo com a formula agora}
